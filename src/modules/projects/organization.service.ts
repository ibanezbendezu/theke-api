...
export class OrganizationService {
  ...
  private async project(accountId: string, projectId: string) { const [value] = await this.database.db.select({ id: projects.id }).from(projects).where(and(eq(projects.id, projectId), eq(projects.accountId, accountId))).limit(1); if (!value) throw new NotFoundException('Proyecto no encontrado.'); return value; }
  ...
  async list(accountId: string, projectId: string) {
    ...
    const resourceRows = await this.database.db.select({ id: projectResources.id, resourceId: resources.id, folderId: projectResources.folderId, title: resources.title, description: resources.description, type: resources.type, updatedAt: resources.updatedAt }).from(projectResources).innerJoin(resources, and(eq(resources.id, projectResources.resourceId), eq(resources.accountId, accountId))).where(eq(projectResources.projectId, projectId));
