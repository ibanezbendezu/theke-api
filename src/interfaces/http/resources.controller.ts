...
export class ResourcesController {
  ...
  @Get() list(@AuthContext() auth: AuthContextValue, @Query('type') type?: string, @Query('status') status?: string, @Query('cursor') cursor?: string, @Query('q') query?: string, @Query('projectId') projectId?: string, @Query('folderId') folderId?: string) { return this.accountId(auth).then(accountId => this.resources.list(accountId, { type, status, cursor, query, projectId, folderId })); }
