...
export class RelationsController {
  ...
  @Post('diagrams/:id/relations') async create(@AuthContext() auth: AuthContextValue, @Param('id', new ParseUUIDPipe()) id: string, @Body() body: Parameters<RelationService['create']>[2]) { return { data: await this.relations.create(await this.accountId(auth), id, body) }; }
}
