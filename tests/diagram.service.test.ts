...
import { accounts, diagramRevisions, diagrams, memberships, operationReceipts, projects, users } from '../src/infrastructure/database/schema.js';
...
describe.runIf(Boolean(process.env.DATABASE_URL))('diagramas con PostgreSQL', () => {
  ...
  it('crea vacío, renombra, duplica la composición y archiva con impacto', async () => {
    ...
    const created = await service.create(accountId, projectId, 'Mapa'); expect(created.document.nodes).toEqual([]); const document = { schemaVersion: 1, nodes: [{ id: 'n1', position: { x: 0, y: 0 }, data: { resourceId: crypto.randomUUID() } }], edges: [], viewport: { x: 0, y: 0, zoom: 1 } };
