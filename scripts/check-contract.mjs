...
const apiBytes = readFileSync(new URL('../packages/contracts/openapi.json', import.meta.url));
const webBytes = readFileSync(new URL('../../theke-web/src/api/contract/openapi.json', import.meta.url));
const digest = createHash('sha256').update(apiBytes).digest('hex');
...
if (digest !== createHash('sha256').update(webBytes).digest('hex')) throw new Error('Drift contractual entre theke-api y theke-web');
